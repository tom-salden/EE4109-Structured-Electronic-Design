...
text2html('A convenient addition to choosing these two topologies is that the output of this two-stage amplifier is inverted in comparision to the input. The reason is that the differential pair inverts the voltage. This means that the output of the stages could directly drive a negative-feedback network.')

head2html('Feedback topology')
text2html('Next to the amplifier, itself, negative feedback can be decided. Advantages of using negative feedback are that ')